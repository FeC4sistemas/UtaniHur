...
export default function App() {
  ...
  const activeChips = useMemo<ActiveChip[]>(() => {
    ...
    if (filters.hireling) chips.push({ key: 'hireling', label: 'Hireling' })
    if (filters.preySlot) chips.push({ key: 'preySlot', label: 'Prey Slot' })
    if (filters.weeklyTask) chips.push({ key: 'weeklyTask', label: 'Weekly Task Slot' })
    if (filters.goldPouch) chips.push({ key: 'goldPouch', label: 'Gold Pouch' })
    ...
    filters.quests.forEach(qn => chips.push({ key: `quest:${qn}`, label: qn }))
    return chips
